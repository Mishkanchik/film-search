...
export const MovieDetailsPage = () => {
  ...
  const { fetchMovieDetails, fetchWatchProviders, fetchMovieVideos, TMDB_IMAGE_BASE } = useTmdbApi();
  ...
  useEffect(() => {
    ...
    loadMovie();
  }, [id, fetchMovieDetails, fetchWatchProviders, fetchMovieVideos]);
  ...
  if (!movie) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh' }}>
        <Typography variant="h4" sx={{ color: 'white' }}>Movie not found</Typography>
      </Box>
    );
  }
  ...
  // Get US providers (or first available)
  const availableProviders = watchProviders?.results?.US || watchProviders?.results?.UA || null;
        ...
        >
          ...
          <Container maxWidth="xl" sx={{ position: 'relative', pt: 3 }}>
            <Button
              ...
              onClick={() => navigate(-1)}
              sx={{ color: 'white', borderColor: 'white', '&:hover': { borderColor: '#9c27b0', color: '#9c27b0' } }}
              variant="outlined"
            >
              Back
            </Button>
          ...
          <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 6 }}>
            ...
            <Box sx={{ flexGrow: 1 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
                ...
                {movie.runtime > 0 && (
                  <Chip label={`${movie.runtime} min`} color="secondary" />
                )}
